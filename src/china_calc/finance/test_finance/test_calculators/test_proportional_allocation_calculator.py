...
from china_calc.finance.calculators.proportional_allocation_calculator import ProportionalAllocationCalculator
...
class TestProportionalAllocationCalculator(TestCase):
    ...
    def test_rejects_negative_total_amount(self):
        with self.assertRaisesRegex(
            ValueError,
            "Распределяемая сумма не может быть отрицательной"
        ):
            result = ProportionalAllocationCalculator.allocate(
                items=self.items,
                total_amount=Decimal('-100'),
    ...
    def test_zero_total_basis(self):
        with self.assertRaisesRegex(
            ValueError,
            "Общая база распределения должна быть больше 0"
        ):
            result = ProportionalAllocationCalculator.allocate(
                items=self.items,
                total_amount=Decimal('100'),
    ...
    def test_none_basis_item(self):
        with self.assertRaisesRegex(
            ValueError,
            "Для товара не указана база распределения"
        ):
            result = ProportionalAllocationCalculator.allocate(
                items=self.items,
                total_amount=Decimal('100'),
